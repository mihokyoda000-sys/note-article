...
**6時間ごと**(日本時間の 2時・8時・14時・20時ごろ)に GitHub Actions が note.com のダッシュボード
...
![note 記事別のアクセス数・スキ](docs/articles.png)
...
| ファイル | 内容 |
|---|---|
| `data/stats.csv` | 取得ごと(6時間ごと)のサマリー(全体ビュー・スキ・コメント・記事数) |
| `data/raw/日時.json` | その回の記事別の生データ(あとで記事別の分析にも使えます) |
| `docs/stats.png` | 累計ビューの推移・1日ごとの増加数・6時間ごとの増加数(直近7日)のグラフ |
| `docs/articles.png` | 記事別のグラフ(ビューの推移 上位5記事・累計ビュー/スキのランキング 上位10記事) |

### 注意事項・困ったとき

- **時刻ぴったりには動かないことがあります。** GitHub Actions の仕様で、混雑時は数分〜数十分遅れます。
...
- 取得は6時間に1回・自分のアカウントのデータのみで、note の非公式 API を利用しています。
